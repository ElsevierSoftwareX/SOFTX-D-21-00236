classdef TA_Circular_HX_Nonideal<TA_HX_Nonideal
    ...
    methods
        %% constructor
        function obj = TA_Circular_HX_Nonideal(name,system,length,D,A,A_solid,solid,heat)
            obj@TA_HX_Nonideal(name,system,length,solid,heat)
            obj.Diameter=D;
            obj.Area=A;
            obj.A_Solid=A_solid;
        end
   ...
   %% set method for Porosity
        ...
        function T_solid=Calculate_Solid_Temperature(obj)
            ...
            gas=char(obj.System_H.Inert);
            k=refpropm('L','T',T_m,'P',p_m/1000,gas);
            alpha=0.0001*refpropm('%','T',T_m,'P',p_m/1000,gas);%m2/s

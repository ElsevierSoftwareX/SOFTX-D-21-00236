...
classdef TA_Duct_Nonideal <TA_Component
    ...
    methods
        %% constructor
        function obj = TA_Duct_Nonideal(name,system,length)
        ...
        function output=ODEfun(obj,input)
            ...
            %F(1)--dP1/dx,F(2)--dU1/dx,F(3)=dTm/dx
             F=[-(1i*omega*ldx+rnu)*input(2);-1i*omega*A/rho/sp^2*(1+(gamma-1)*fk)*input(1);0];
            output=F(:);
            %% internal function
